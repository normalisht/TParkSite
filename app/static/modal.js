...
(function(){
    ...
    function EasyTogglerHandler(event){
        ...
        if(EY_BTN){
            ...
            try{document.querySelector(ey_target).classList.toggle(ey_class)}
            catch(ey_error){console.warn('EasyToggler.js: Блок '+ey_target+' не существует')}
        }
        if(!EY_BTN){
            ...
            Array.from(ey_rcoe_block_targets).forEach.call(ey_rcoe_block_targets,function(ey_rcoe_block_target){
                ...
                if(!event.targetclosest(ey_rcoe_block)){
                    try{
